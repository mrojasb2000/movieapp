...
// Register creates a service record in the in-memory.
...
func (r *Registry) ReportHealthyState(instanceID string, serviceName string) error {
	...
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
		return errors.New("service isn't registered yet")
	}
	if _, ok := r.serviceAddrs[serviceName][instanceID]; !ok {
		return errors.New("service instance isn't registered yet")
	}
